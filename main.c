...
void bonds(int *px,int *py,int*ex,int*ey){
    if (*px <= 0)
            *px = windowX-32;
        if (*px >= windowX)
            *px = 32;
    ...
    // clear the background -> draw -> go to the second frame -> clear the background -> draw
        ...
        if (state == Game) {
            ...
            if (dist > 64) {
                ...
                ey += dt*velocity*GetRandomValue (-ey, windowY-ey); // mean value: windowY/2a
                if(ex<px)
                ex+=velocity*dt*func.x;
                else ex-=velocity*dt*func.x;
                if(ey<py)
                ey+=velocity*dt*func.y;
                else ey-=velocity*dt*func.y;
