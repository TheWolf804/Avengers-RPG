...
typedef struct Game {
    ...
    float bombInterval, bombTimer,appleTotal; //I suggest this rename
};
...
typedef struct Apple{
   float x[MAX_APPLES],y[MAX_APPLES]; // watch out for capitalisation
   int number;
...
struct Game game;
Apple apples; // watch out for name collisions
Bomb bombs;
...
void keys (float *px, float *py, float *speed, int *size, int *circleX, int *circleY) {
        ...
        if (*px-*circleX< 100 && *py-*circleY < 100) {
            if (IsKeyPressed (KEY_F)) {
                ...
                PlaySound (eat);
                game.health++;
                  // this isnt a correct usage, we are supposed
                  // to have a fixed number of apples, and we should
                  // also generate their coordinates once
...
int main () {
    ...
    // incorrect place to compute collisions, you do this in the game loop
         // difficultySelector must be before this code above
    // we need to compute the coordinates for all the apples BEFORE we start the
    //game, otherwise, it will compute the values constantly and we dont want that
    ...
    // clear the background -> draw -> go to the second frame -> clear the background -> draw
        ...
        if (state == Game) {
            ...
            // watch out for commas(,) in argument lists when calling a function
            bonds(&px,&py, &ex, &ey);
            ...
            */
            ...
            if(diff>=Practise){
                for(int i=0;i<=apples.number;i++){ 
                    ...
                    if(collision<=64&&apples.used[i]==false&&IsKeyPressed(KEY_F)){
                        apples.used[i]=true;
                        game.health++; 
                        PlaySound(eat);// bad brackets
                    }
                } // please use indentation (press Tab on your keyboard), its hard to read otherwise
            }
        }
        ...
        BeginDrawing(); //drawing block
            ...
            if (state <= Dead) {
                ...
                if (diff >= Insane) {
                 ...
                 //if(apples.number<=0.1){ // incorrect, this code is only valid for
                 // a timer
                   ...
                   // these 2 lines only generate coordinates for the last
                   // apple in the list of apples, rather than for all of them
                   
                   
                   //apples.number++;
                   //if(apples.used[i]==false) // if you use i, you must have
                   // a variable i declared, such as the one in a for loop
                     for (int i = 0; i < apples.number; i++)
